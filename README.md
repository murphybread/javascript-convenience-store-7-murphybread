# javascript-convenience-store-7-murphybread
...
- [x] 3.1 프로모션 상품이 없는 경우 일반 재고 확인
- [x] 3.2 프로모션이 가능한 경우 재고 확인
  - []3.2.1 프로모션 수량에 문제가 없는 경우
  - [x]3.2.2 고객이 추가로 가져오면 프로모션이 가능한 경우
  - []3.2.3 프로모션 재고가 부족한 경우 일반 재고 알림
...
- [x] 프로모션은 N개 구매 시 1개 무료 증정(Buy N Get 1 Free)의 형태로
- [x] 1+1 또는 2+1 프로모션이 각각 지정된 상품에 적용되며, 동일 상품에 여러 프로모션이 적용되지 않는다.
      진행된다.
  - 반짝할인 1+1
  - MD추천상품 1+1
  - 탄산2+1
- [x] 프로모션 기간 중이라면 프로모션 재고를 우선적으로 차감하며
